...
class mamonAnn():
    ...
    def farowrds(self , X,factive,inneractive,lactive):
        #print len(self.hedins)

        for i in xrange(len(self.hedins)):

            if i == 0:
                #print("doing " ,inneractive[i])
        ...
        #print("doing " ,lactive)
    def back_train(self ,X , T , lr , reg , factive,inneractive,lactive,zerbias):
        regularization = reg
        ...
        self.W[-1] += lr * dc
        self.b[-1] -= lr*(S_y.sum() - regularization * self.b[-1])  * zerbias
        S_yold = S_y
        W2 = self.W[-1]
        # gradient descent
        prev_layer = self.out
        for i in xrange(len(self.hedins)-1,-1,-1):
            ...
            S_flayer = S_anylayer(self.hedins[i] , W2 , S_y , actvite , prev_layer)
            self.W[i] += lr *(in_layer.T.dot(S_flayer)  -( regularization * self.W[i] ) )#delta_Fast(lr ,T , self.out , in_layer, 'any' ,  self.hedins[i] , W2)
            self.b[i] += lr*(S_flayer.sum(axis=0)  - regularization * self.b[i]  ) * zerbias
            W2 = self.W[i]
            prev_layer = self.hedins[i]
            S_y = S_flayer
    def fit(self,X,T,forlong , prnt,lr,batch ,reg , factive , mid , last , LL,zerbias):
        ...
        for trn in xrange(forlong):
            ...
            if er != last_error_rate:
                last_error_rate = er
            Y_old = self.out
            self.back_train(X , T,lr ,reg ,factive , mid , last ,zerbias)
...
def S_anylayer(layer , layer_w , next_s , actvite , prev_layer):
    ...
    if len(layer_w.shape) == 1:
        for k in xrange(K):
            for i in xrange(layer_w.shape):
                ...
                coz = dos + S[k,i]
                S[k,i] = coz
                joso = S[k,i] * F_hate(layer[k,i],actvite)
                S[k,i] = joso
    else:
        I , J = layer_w.shape
        for k in xrange(K):
